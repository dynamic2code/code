...
## returns number of bytes written
  ...
  def Write(self, file_inode_number, offset, data):
    ...
    while bytes_written < len(data):
      ...
      INMEM_CACHE[block_number] = {
        "gencnt" : file_inode.inode.gencnt,
        "block" : block
      }
      #print (file_inode.inode.__dict__, block_number)
  ...
  def Read(self, file_inode_number, offset, count):
    ...
    while bytes_read < bytes_to_read:
      ...
      # first, we read the whole block from raw storage
      if block_number in INMEM_CACHE and file_inode.inode.gencnt == INMEM_CACHE[block_number]["gencnt"]:
        #print (file_inode.inode.gencnt, block_number, INMEM_CACHE[block_number])
        block = INMEM_CACHE[block_number]["block"]
      else:
        block = file_inode.RawBlocks.Get(block_number)
        INMEM_CACHE[block_number] = {
          "gencnt" : file_inode.inode.gencnt,
      ...
      while current_position < end_position:
        ...
        if entryname_padded == padded_filename:
          ...
          if (last_block_index == block_index) and (last_block_end_position == current_position):
            ...
            INMEM_CACHE[last_block_index] = {
              "gencnt" : dir_inode.inode.gencnt,
              "block" : last_block
            }
          # write the block back
          self.RawBlocks.Put(block_index,block)
          INMEM_CACHE[block_index] = {
            "gencnt" : dir_inode.inode.gencnt,
            "block" : block
          }          
          # update inode size
